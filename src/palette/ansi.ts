 ...
 */
export const CLAUDE_CODE_MESSAGE_SLOTS: Readonly<Record<Appearance, ClaudeCodeMessageSlots>> = {
...
interface ClaudeCodeMessageHexes {
  readonly textHex: string;
  readonly messageBackgroundHex: string;
}
...
function moveAtOwnHue(hex: string, referenceHex: string, targetRatio: number, isLighterThanReference: boolean): string {
 ...
 */
function repairClaudeCodeMessagePair(
  textHex: string,
  messageBackgroundHex: string,
  groundHex: string,
  appearance: Appearance,
): ClaudeCodeMessageHexes {
  if (contrastRatio(textHex, messageBackgroundHex) >= TEXT_MIN_RATIO) return { textHex, messageBackgroundHex };
...
export function repairAnsiSlots(scheme: Scheme): AnsiRepairReport {
  ...
  const repairedSlots = new Set<AnsiSlotName>();
  ...
  for (const slotName of ANSI_SLOT_NAMES) {
    ...
    if (repaired.wasRepaired) repairedSlots.add(slotName);
  }
  ...
  if (message.textHex !== slots[messageSlots.text]) repairedSlots.add(messageSlots.text);
  if (message.messageBackgroundHex !== slots[messageSlots.messageBackground]) repairedSlots.add(messageSlots.messageBackground);
