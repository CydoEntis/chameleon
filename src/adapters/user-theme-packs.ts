...
type PackDirectoryResult = { readonly pack: ThemePack } | { readonly warning: string };

/**
 * Reads one pack sub-directory. Every failure — a missing manifest, invalid
 * JSON, a manifest that fails UserPackManifestSchema, a scheme that still
 * cannot clear its floor after repair — becomes a named warning rather than
 * a thrown error, so one bad pack never stops the others loading.
 */
function readPackDirectory(packDirPath: string, packDirName: string): PackDirectoryResult {
  ...
  try {
    ...
    const pack = buildThemePack(manifest.scheme, manifest.family ?? manifest.scheme.name);
    return { pack };
  } catch (error) {
 ...
 * contrast floors a bundled one does — see CLAUDE.md, "Run a user pack
 * through the same contrast floors as a bundled one."
 *
