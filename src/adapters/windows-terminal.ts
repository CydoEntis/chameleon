...
function upsertSchemesEntry(settingsPath: string, text: string, scheme: Scheme): string {
  ...
  const schemesNode = findNodeAtLocation(parseJsoncTree(settingsPath, text), ["schemes"]);
  ...
  const container = findNodeAtLocation(parseJsoncTree(settingsPath, dedupedText), ["schemes"]);
...
function upsertDefaultColorScheme(settingsPath: string, text: string, schemeName: string): string {
  const eol = detectLineEnding(text);
  const defaultsNode = findNodeAtLocation(parseJsoncTree(settingsPath, text), ["profiles", "defaults"]);
  ...
  const container = findNodeAtLocation(parseJsoncTree(settingsPath, dedupedText), ["profiles", "defaults"]);
...
function upsertTopLevelTheme(settingsPath: string, text: string, appearance: Appearance): string {
  const eol = detectLineEnding(text);
  const root = parseJsoncTree(settingsPath, text);
  if (root.type !== "object") {
  ...
  const container = parseJsoncTree(settingsPath, dedupedText);
