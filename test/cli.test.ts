 ...
 * which real pack a colour came from, and the 63 bundled packs are not
...
describe("renderPickerRow", () => {
  ...
  describe("the highlighted-row marker", () => {
    ...
    it("covers all 63 bundled packs", () => {
      expect(allEntries.length).toBe(63);
    });
...
describe("renderPickerFrame", () => {
  ...
  // three digits, not just the 63 bundled packs. The gutter is sized from
...
describe("createSettledFileTargetPreview", () => {
  ...
  // one of the 63 bundled packs, with no pause between rows, must perform a
