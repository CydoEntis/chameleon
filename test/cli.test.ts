 ...
 * which real pack a colour came from, and the 32 bundled packs are not
...
describe("renderPickerRow", () => {
  ...
  describe("the highlighted-row marker", () => {
    ...
    it("covers all 32 bundled packs", () => {
      expect(allEntries.length).toBe(32);
    });
...
describe("renderPickerFrame", () => {
  ...
  // three digits, not just the 32 bundled packs. The gutter is sized from
...
describe("createSettledFileTargetPreview", () => {
  ...
  // one of the 32 bundled packs, with no pause between rows, must perform a
