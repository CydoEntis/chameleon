...
import { MUTED_MIN_RATIO, ROLES, SELECTION_MIN_VISIBLE_RATIO, TEXT_MIN_RATIO } from "../../src/constants.js";
...
describe("buildThemePack", () => {
  ...
  it("ships the raw scheme as the windows-terminal payload, apart from a resolved selectionBackground", () => {
    const scheme = readVendoredScheme("Dracula.json");
    const pack = buildThemePack(scheme, "Dracula", ATTRIBUTION);

    const wtPayload = pack.payloads["windows-terminal"];
    expect(wtPayload).toEqual({ ...scheme, selectionBackground: wtPayload.selectionBackground });
