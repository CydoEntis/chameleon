...
import {
  HERDR_BUILTIN_GROUNDS,
  HERDR_DERIVED_SURFACE_TOKENS,
  createHerdrAdapter,
  herdrMatchesRoleHexes,
  nearestHerdrBuiltinThemeNameFor,
  undoHerdr,
} from "../../src/adapters/herdr.js";
...
// CHM-48: the sidebar's selected row paints subtext0 directly on top of
// active_row_bg, a mid-tone background Chameleon mixes for itself — and
// nothing before this measured that pairing, or any of the surface scale's
// other tokens, against the text Herdr actually renders on top of them. 22
// of 26 bundled packs measured subtext0-on-active_row_bg under its own floor
// of 3.0 before this fix, some as low as 1.07. These tests pin the fix
// against the real 26 committed under themes/, generated by
// tools/build-theme-packs.ts — not a re-derived copy — and enumerate every
// token HERDR_DERIVED_SURFACE_TOKENS names, not a hand-picked subset of it,
// so a token added there later is checked automatically rather than by
// someone remembering to update this test too.
describe("herdr adapter — surface tokens vs text and subtext0 (CHM-48)", () => {
  function customTokensFor(scheme: Scheme, slug: string): Record<string, string> {
    const configDir = mkdtempSync(path.join(tmpdir(), "chameleon-herdr-surfaces-"));
    const configPath = path.join(configDir, "config.toml");
    writeFileSync(configPath, '[theme]\nname = "builtin"\n', "utf8");
    try {
  ...
  it("clears text at 4.5 and subtext0 at 3.0 against every one of HERDR_DERIVED_SURFACE_TOKENS, for every bundled pack", () => {
    const packs = loadCuratedThemePacks();
    expect(packs.length).toBeGreaterThan(0);
    expect(HERDR_DERIVED_SURFACE_TOKENS.length).toBeGreaterThan(1);
    ...
    for (const pack of packs) {
      ...
      if (!text || !subtext0) throw new Error(`"${pack.manifest.slug}" wrote no text/subtext0 tokens`);

      for (const surfaceToken of HERDR_DERIVED_SURFACE_TOKENS) {
        const surfaceHex = customTokens[surfaceToken];
        if (!surfaceHex) throw new Error(`"${pack.manifest.slug}" wrote no "${surfaceToken}" token`);

        expect(contrastRatio(text, surfaceHex)).toBeGreaterThanOrEqual(TEXT_MIN_RATIO);
        expect(contrastRatio(subtext0, surfaceHex)).toBeGreaterThanOrEqual(MUTED_MIN_RATIO);
        pairsChecked += 2;
      }
    }

    // Asserted so a future change that quietly narrows the surfaces or packs
    // checked — back down to just ground, say — fails this test by itself,
    // rather than the coverage silently shrinking underneath a check that
    // still reads as passing (CHM-48's own root cause).
    expect(pairsChecked).toBe(packs.length * HERDR_DERIVED_SURFACE_TOKENS.length * 2);
  });

  // The four packs CHM-48 names by hand as the worst measured — the reporter
  // is on nord-dark. Ratios below are active_row_bg's own subtext0-vs-surface
  // measurement before this fix (see this ticket's own body).
  const WORST_FOUR_FIXTURES = [
    { slug: "dracula-dark", preRepairSubtextRatio: 1.07 },
    { slug: "monokai-dark", preRepairSubtextRatio: 1.07 },
    { slug: "night-owl-dark", preRepairSubtextRatio: 1.23 },
    { slug: "nord-dark", preRepairSubtextRatio: 1.33 },
  ];

  it.each(WORST_FOUR_FIXTURES)(
    "repairs $slug's active_row_bg, which measured $preRepairSubtextRatio for subtext0-on-surface before CHM-48",
    ({ slug, preRepairSubtextRatio }) => {
      const packs = loadCuratedThemePacks();
      const pack = packs.find((candidate) => candidate.manifest.slug === slug);
      if (!pack) throw new Error(`fixture pack not found: ${slug}`);

      const customTokens = customTokensFor(pack.payloads["windows-terminal"], pack.manifest.slug);
      const activeRowBgHex = customTokens["active_row_bg"];
      const subtext0Hex = customTokens["subtext0"];
      if (!activeRowBgHex || !subtext0Hex) throw new Error(`"${slug}" wrote no active_row_bg/subtext0 tokens`);

      const achievedRatio = contrastRatio(subtext0Hex, activeRowBgHex);
      expect(achievedRatio).toBeGreaterThanOrEqual(MUTED_MIN_RATIO);
      expect(achievedRatio).toBeGreaterThan(preRepairSubtextRatio);
    },
  );
});
