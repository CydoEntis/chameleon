...
// output, for every one of the 32 bundled packs.
...
// config.toml for every one of the 32 bundled packs — not a re-derivation,
...
describe("herdr adapter — CHM-79's declared contrast inventory, every bundled pack", () => {
  ...
  it("clears every declared pair for every one of the 32 bundled packs — the gate this ticket adds, run against real written output", () => {
    const packs = loadCuratedThemePacks();
    expect(packs.length).toBe(32);
