...
import { z } from "zod";
import { acquireLock, activePackRoleHexes, activePackStatuslineMeterHexes, addSegment, ANSI_SLOT_NAMES, applyThemePack, beginThemePreview, buildLayoutSegment, createDefaultOhMyPoshAdapter, createWindowsTerminalAdapter, currentGitBranch, currentLockHolder, currentPack, didAnyTargetFail, disableClaudeCodeStatusLine, enableClaudeCodeStatusLine, findFamilySibling, isKnownRole, isSegmentType, layoutBlocksOnSide, loadAllThemePacks, moveSegmentBetweenBlocks, nextPackSlug, previewThemePackToFileTargets, prevPackSlug, readOhMyPoshLayout, removeDeadWindowsTerminalSchemeForks, removeSegment, reorderSegment, reseedOhMyPoshOwnedConfig, restoreOriginal, resyncInterruptedPreview, ROLES, runDoctorChecks, SEGMENT_TYPES, undoAppliedPack, VERSION, writeOhMyPoshLayout, } from "./index.js";
/**
 ...
 */
export function formatThemeLine(loaded) {
 ...
 */
function formatTargetLine(check) {
    if (!check.isApplicable)
        return `${check.target}: not available on this platform`;
 ...
 * the ticket."
 */
function formatNerdFontLine(nerdFont) {
 ...
 */
export function formatClaudeCodeRestartNote(isInstalled) {
    if (!isInstalled)
        return undefined;
 ...
 */
export function formatClaudeCodeStatusLineLine(statusLine) {
    if (!statusLine)
        return undefined;
 ...
 */
export function formatOhMyPoshOwnedConfigLine(owned) {
    if (!owned)
        return undefined;
...
function formatDriftedTargets(driftedTargets) {
...
function matchesVerbFor(driftedTargets) {
 ...
 */
function isPackUnloadable(report) {
 ...
 */
function formatPreviewInFlightNotice(driftedTargets) {
 ...
 */
export function formatDriftLine(drift) {
    if (!drift)
        return "drift: no pack has been applied yet — nothing to compare";
    if (isPackUnloadable(drift))
        return `cannot check drift: pack "${drift.slug}" is no longer available`;
    if (drift.driftedTargets.length === 0)
        return `drift: none — every detected target matches "${drift.slug}"`;
    if (drift.previewInFlight)
        return `drift: ${formatPreviewInFlightNotice(drift.driftedTargets)}`;
 ...
 */
export function formatContrastFailureLines(contrast) {
    const herdrLines = (contrast.herdr ?? []).map((failure) => `${failure.pair.label} measures ${failure.ratio.toFixed(2)}, below its floor of ${failure.pair.minRatio}`);
    const windowsTerminalLines = (contrast.windowsTerminal ?? []).map((failure) => `${failure.pair.label} measures ${failure.ratio.toFixed(2)}, below its floor of ${failure.pair.minRatio}`);
 ...
 */
export function parseStatuslinePayload(rawStdin) {
    try {
 ...
 */
function statuslineDirectory(payload) {
...
function statuslineContextPercent(payload) {
 ...
 */
function statuslineRateLimitPercent(payload, window) {
 ...
 */
function paintStatuslineSegment(hex, text) {
...
function buildMeterBar(wholePercent) {
 ...
 */
function buildMeterSegment(meterHex, errorHex, label, wholePercent) {
 ...
 */
export function buildStatuslineText(payload, roleHexes, meterHexes, gitBranch) {
    ...
    if (gitBranch !== undefined)
        segments.push(paintStatuslineSegment(roleHexes?.success, gitBranch));
    const contextPercent = statuslineContextPercent(payload);
    if (contextPercent !== undefined)
        segments.push(buildMeterSegment(meterHexes?.context, roleHexes?.error, "context", contextPercent));
    const fiveHourPercent = statuslineRateLimitPercent(payload, "five_hour");
    if (fiveHourPercent !== undefined)
        segments.push(buildMeterSegment(meterHexes?.fiveHour, roleHexes?.error, "5h", fiveHourPercent));
    const sevenDayPercent = statuslineRateLimitPercent(payload, "seven_day");
    if (sevenDayPercent !== undefined)
        segments.push(buildMeterSegment(meterHexes?.sevenDay, roleHexes?.error, "7d", sevenDayPercent));
...
function requireFlagValue(args, flagName) {
...
function parseBlockName(args, flagName) {
...
function parseRole(args, flagName) {
    const rawValue = requireFlagValue(args, flagName);
...
function parseOptionalRole(args, flagName) {
...
function parseSegmentType(args) {
...
function parseIndex(args, flagName) {
    const rawValue = requireFlagValue(args, flagName);
...
function parseOptionalIndex(args, flagName) {
 ...
 */
function parseBlockIndex(args, layout, alignment, flagName) {
...
function runEditMove(args) {
    ...
    process.stdout.write(`moved segment ${atIndex} from block ${fromBlockIndex} of the ${fromBlock} side to block ${toBlockIndex} of the ${toBlock} side\n`);
    return 0;
}
/**
 ...
 */
function runEdit(argv) {
    const [subcommand, ...rest] = argv;
    try {
        if (subcommand === "add")
            return runEditAdd(rest);
        if (subcommand === "remove")
            return runEditRemove(rest);
        if (subcommand === "reorder")
            return runEditReorder(rest);
        if (subcommand === "move")
            return runEditMove(rest);
 ...
 */
export function formatNoteworthyResultLines(results) {
...
function printNoteworthyResults(results) {
...
export function formatLockHeldMessage(holder) {
    if (!holder)
        return "chm: another chm process is writing right now — try again in a moment";
 ...
 */
function runWithWriteLock(commandLabel, body) {
    ...
    return runWithWriteLock(`chm ${slug}`, () => {
        try {
            ...
            if (!report.isFullyApplied) {
                process.stderr.write(`chm: ${report.slug} was only partially applied — the state file was left unchanged, and \`chm current\`/\`chm doctor\` may now report drift until this is fixed\n`);
                return 1;
            }
            return 0;
        }
 ...
 */
function reportRestoreResults(headline, results) {
...
function runClean() {
    return runWithWriteLock("chm clean", () => {
        try {
            ...
            process.stdout.write(removedCount === 0
 ...
 */
function runFamilySwitch(appearance) {
    try {
...
export function formatPreviewInProgressLine(holder) {
...
function runCurrent(args) {
    ...
    if (current.driftedTargets.length > 0) {
        ...
        process.stderr.write(`chm current: drifted — ${formatDriftedTargets(current.driftedTargets)} no longer ${matchesVerbFor(current.driftedTargets)} "${current.slug}"\n`);
        return 1;
    }
    return 0;
}
export function toPickerEntry(loaded) {
    const roleHexes = loaded.pack.payloads["oh-my-posh"];
    return {
...
const APPEARANCE_GROUP_ORDER = ["dark", "light"];
/**
 ...
 * discovers.
 */
export function groupedByAppearance(entries) {
...
export function appearanceGroupHeading(appearance, entries) {
...
const OSC_PALETTE_INDEX_BY_SLOT = {
    black: 0,
    red: 1,
    green: 2,
    yellow: 3,
...
function oscSetPaletteColor(paletteIndex, hex) {
    return `\x1b]4;${paletteIndex};${hex}${OSC_TERMINATOR}`;
}
function oscSetSpecialColor(oscCode, hex) {
    return `\x1b]${oscCode};${hex}${OSC_TERMINATOR}`;
}
function oscResetPaletteColor(paletteIndex) {
    return `\x1b]104;${paletteIndex}${OSC_TERMINATOR}`;
}
function oscResetSpecialColor(oscCode) {
 ...
 * not file writes."
 */
export function buildTerminalPreviewSequence(scheme) {
 ...
 */
export function buildTerminalResetSequence() {
 ...
 */
export function createSettledFileTargetPreview(applyToFileTargets, debounceMs = FILE_PREVIEW_DEBOUNCE_MS) {
    let pendingTimer;
    function cancel() {
...
function matchesPickerFilter(entry, filterText) {
    if (filterText === "")
        return true;
 ...
 */
function pickerRowDotHexes(entry) {
...
function paintedPickerRowDots(entry) {
...
function pickerRowMarker(isHighlighted, isApplied) {
    if (isHighlighted)
        return PICKER_HIGHLIGHTED_MARKER;
    if (isApplied)
        return PICKER_APPLIED_MARKER;
 ...
 */
function formatPickerRowNumber(displayNumber, gutterDigits) {
 ...
 */
function pickerRowContent(entry, displayNumber, gutterDigits, dotsText) {
 ...
 */
function computePickerRowLayout(entries) {
    const gutterDigits = Math.max(1, String(entries.length).length);
    const plainDots = plainPickerRowDots();
    const contentWidth = entries.reduce((widestSoFar, entry, index) => Math.max(widestSoFar, pickerRowContent(entry, index + 1, gutterDigits, plainDots).length), 0);
 ...
 * never shown.
 */
export function renderPickerRow(entry, position, layout) {
 ...
 */
function pickerWindowStart(totalCount, highlightedIndex, maxVisibleRows) {
    if (totalCount <= maxVisibleRows)
        return 0;
 ...
 */
export function renderPickerFrame(entries, highlightedIndex, filterText, appliedSlug) {
    const filterLine = filterText === "" ? [] : [`filter: ${filterText}`];
    if (entries.length === 0)
        return [PICKER_HEADER_LINE, ...filterLine, "  no matches"];
    ...
    const rowLines = entries.slice(windowStart, windowEnd).flatMap((entry, windowIndex) => {
        ...
        const row = renderPickerRow(entry, { displayNumber: index + 1, isHighlighted: index === highlightedIndex, isApplied: entry.slug === appliedSlug }, layout);
...
function clearPickerFrame(lineCount) {
    if (lineCount === 0)
        return;
 ...
 */
export function shouldRestoreOriginalSelectionOnExit(originalSlug, currentActiveSlug) {
 ...
 */
async function runInteractivePicker(packs, originalSlug, releaseLock) {
    ...
    return new Promise((resolve) => {
        ...
        function previewHighlighted() {
            ...
            if (entry === undefined || entry.slug === lastPreviewedSlug)
                return;
        ...
        function applyFilter(nextFilterText) {
        ...
        function restoreTerminalPreview() {
        ...
        function cancel() {
            ...
            if (shouldRestoreOriginalSelectionOnExit(originalSlug, currentPack()?.slug)) {
                ...
                process.stderr.write(`chm: the active theme changed while the picker was open — leaving it as "${currentPack()?.slug ?? "nothing"}"\n`);
            }
            releaseLock();
            stopListening();
            resolve(undefined);
        ...
        function onKeypress(inputChar, key) {
            ...
            if (key?.name === "backspace")
                return applyFilter(filterText.slice(0, -1));
            if (inputChar && /^[\x20-\x7e]$/.test(inputChar))
                return applyFilter(filterText + inputChar);
        }
 ...
 * piping still works."
 */
export function wantsPlainThemeList(args, isStdinTTY, isStdoutTTY) {
 ...
 */
function levenshteinDistance(queryText, candidateKey) {
    const rowCount = queryText.length + 1;
    const columnCount = candidateKey.length + 1;
    const distances = Array.from({ length: rowCount }, (_unused, rowIndex) => new Array(columnCount).fill(rowIndex));
    ...
    for (let rowIndex = 1; rowIndex < rowCount; rowIndex += 1) {
        for (let columnIndex = 1; columnIndex < columnCount; columnIndex += 1) {
            ...
            distances[rowIndex][columnIndex] = Math.min(distances[rowIndex - 1][columnIndex] + 1, distances[rowIndex][columnIndex - 1] + 1, distances[rowIndex - 1][columnIndex - 1] + substitutionCost);
        }
    }
...
function closestThemeByName(packs, normalizedQuery) {
    let closest;
 ...
 */
export function resolveThemeQuery(packs, rawTokens) {
    ...
    if (exactMatches.length === 1)
        return { status: "resolved", loaded: exactMatches[0] };
    if (exactMatches.length > 1)
    ...
    if (prefixMatches.length === 1)
        return { status: "resolved", loaded: prefixMatches[0] };
    if (prefixMatches.length > 1)
        return { status: "ambiguous", candidates: prefixMatches };
...
function formatAmbiguousCandidateLine(loaded) {
 ...
 */
function runApplyByQuery(rawTokens) {
    const { packs, warnings } = loadAllThemePacks();
    for (const warning of warnings) {
        process.stderr.write(`${warning}\n`);
    }
    ...
    if (result.status === "resolved")
        return runApply(result.loaded.pack.manifest.slug);
...
async function main(argv) {
    ...
    if (command === undefined)
        return runUsage();
    if (command === "themes" || command === "pick")
        return runThemes(rest);
    if (command === "doctor")
        return runDoctor();
    if (command === "edit")
        return runEdit(rest);
    if (command === "reseed")
        return runReseed(rest);
    if (command === "clean")
        return runClean();
    if (command === "statusline") {
        if (rest[0] === "on")
            return runStatuslineOn();
        if (rest[0] === "off")
            return runStatuslineOff();
    ...
    if (command === "prev")
        return runPrev();
    if (command === "dark")
        return runFamilySwitch("dark");
    if (command === "light")
        return runFamilySwitch("light");
