...
import { z } from "zod";
import { isKnownRole, ROLES, TEXT_MIN_RATIO } from "../constants.js";
import { repairForegroundAgainstBackgrounds, resolveRoleHexes } from "../palette/repair.js";
import { promptPaletteFor, recoloredHexFor, recoloredLiteralHexFor } from "../palette/role-mapping.js";
import { buildPropertyBlockContent, dedupeConflict, detectLineEnding, findPropertyNode, parseJsonTree, upsertMarkedBlock, } from "./marked-json-edit.js";
import { detectShell, ohMyPoshProfilePathFor, stateDir } from "./platform.js";
 ...
 */
export function ohMyPoshMatchesScheme(config, scheme) {
 ...
 * look for there.
 */
function initShellNamesFor(shell) {
    if (shell === "pwsh")
 ...
 */
function initConfigArgumentPattern(shellNames) {
 ...
 */
function expandPathReferences(rawPath, shell) {
    if (shell === "pwsh") {
        return rawPath.replace(/\$env:(\w+)/gi, (reference, name) => process.env[name] ?? reference);
    }
    const withVariablesExpanded = rawPath.replace(/\$\{(\w+)\}|\$(\w+)/g, (reference, braced, bare) => {
 ...
 */
function withoutOwnedMarkerBlocks(profileText) {
 ...
 */
function configPathFromProfile(profilePath, shell) {
    const shellNames = initShellNamesFor(shell);
    if (shellNames.length === 0 || !existsSync(profilePath))
        return undefined;
 ...
 * real one. See CHM-36.
 */
function resolveConfigPath(configPath, profilePath, shell) {
 ...
 */
function defaultProfilePath(shell = "pwsh") {
 ...
 */
function readOhMyPoshConfig(configPath) {
 ...
 */
function dedupeRootProperty(configPath, text, key) {
 ...
 */
function upsertPaletteTable(configPath, text, paletteTable) {
 ...
 * nearestHueFamilyHue.
 */
function recoloredForeignHex(key, hex, resolvedRoleHexes, targetScheme) {
    if (isKnownRole(key))
        return resolvedRoleHexes[key];
    ...
    if (isLiteralLiftedKey(key))
        return recoloredLiteralHexFor(literalHexFromKey(key) ?? hex, resolvedRoleHexes);
 ...
 */
function recoloredPaletteTable(existingPalette, resolvedRoleHexes, targetScheme, colorsByLiteralKey = {}) {
    ...
    const recoloredExisting = Object.fromEntries(Object.entries(existingPalette ?? {}).map(([key, hex]) => [
        key,
...
function paletteReferencePattern() {
...
function palettesReferencedIn(configText) {
    ...
    for (const match of configText.matchAll(paletteReferencePattern())) {
        ...
        if (referencedRole !== undefined)
            referencedRoles.add(referencedRole);
    }
...
function paletteReferencesIn(fieldValue) {
 ...
 */
function assertNoDanglingPaletteReferences(configPath, configText, paletteTable) {
...
function isGeneratedOverrideKey(key) {
 ...
 */
function overrideKeyFor(sourceKey) {
...
function segmentsOf(rawBlock) {
    if (typeof rawBlock !== "object" || rawBlock === null)
        return undefined;
    const segments = rawBlock["segments"];
...
function withSegmentsTransformed(rawBlock, transformSegment) {
    const segments = segmentsOf(rawBlock);
    if (segments === undefined)
        return rawBlock;
    return {
        ...rawBlock,
        segments: segments.map((rawSegment) => (typeof rawSegment === "object" && rawSegment !== null ? transformSegment(rawSegment) : rawSegment)),
    };
}
...
function isLiftableLiteralForeground(fieldValue) {
...
function literalColorPaletteKeyFor(hex) {
...
function isLiteralLiftedKey(key) {
 ...
 */
function literalHexFromKey(key) {
 ...
 */
const PROMPT_COLOR_BY_SEGMENT_TYPE = {
    session: "blue",
    root: "blue",
    os: "blue",
    shell: "blue",
 ...
 */
function promptColorsByLiteralForegroundKey(rawBlocks) {
    const colorsSeenByKey = new Map();
    for (const rawBlock of rawBlocks) {
        for (const rawSegment of segmentsOf(rawBlock) ?? []) {
            ...
            for (const key of segmentForegroundKeys(segment).filter(isLiteralLiftedKey)) {
                const seen = colorsSeenByKey.get(key) ?? new Set();
                seen.add(color);
                colorsSeenByKey.set(key, seen);
            }
        }
    }
    return Object.fromEntries([...colorsSeenByKey.entries()].filter(([, colors]) => colors.size === 1).map(([key, colors]) => [key, [...colors][0]]));
}
/**
 ...
 */
function liftLiteralForegroundsToPalette(rawBlocks) {
    const paletteAdditions = {};
    const unliftableSegmentTypes = [];
    const blocks = rawBlocks.map((rawBlock) => withSegmentsTransformed(rawBlock, (segment) => {
        const rawForeground = segment["foreground"];
        if (rawForeground === undefined || roleReferencedBy(rawForeground) !== undefined)
            return segment;
...
function segmentBackgroundHexes(segment, paletteTable) {
...
function segmentForegroundKeys(segment) {
...
function withRoleReferenceReplaced(fieldValue, foregroundKey, overrideKey) {
    if (typeof fieldValue !== "string")
        return fieldValue;
...
function segmentWithForegroundRepointed(segment, foregroundKey, overrideKey) {
 ...
 * every apply.
 */
function withGeneratedForegroundReferencesNormalized(segment) {
    return segmentForegroundKeys(segment)
        .filter((foregroundKey) => isGeneratedOverrideKey(foregroundKey))
        .reduce((currentSegment, foregroundKey) => segmentWithForegroundRepointed(currentSegment, foregroundKey, sourceKeyFor(foregroundKey)), segment);
}
...
function withGeneratedBlockForegroundsNormalized(rawBlock) {
 ...
 */
function backgroundSignatureFor(backgroundHexes) {
 ...
 */
function collectSegmentBackgroundHexes(segment, paletteTable, signaturesByForegroundKey) {
    ...
    for (const foregroundKey of segmentForegroundKeys(segment)) {
        const signaturesForKey = signaturesByForegroundKey.get(foregroundKey) ?? new Map();
 ...
 * happened to sit.
 */
function overrideKeysBySignatureFor(foregroundKey, foregroundHex, backgroundHexesBySignature) {
    const repairedBySignature = [...backgroundHexesBySignature.entries()]
        .map(([signature, backgroundHexes]) => [signature, repairForegroundAgainstBackgrounds(foregroundHex, backgroundHexes, TEXT_MIN_RATIO)])
...
function computeForegroundOverrides(signaturesByForegroundKey, paletteTable) {
    const overrideKeysByForegroundKeyAndSignature = new Map();
    const additionalPaletteEntries = {};
    for (const [foregroundKey, backgroundHexesBySignature] of signaturesByForegroundKey) {
        ...
        if (overridesBySignature.size === 0)
            continue;
        const overrideKeysBySignature = new Map();
...
function segmentWithOverridesApplied(segment, paletteTable, overrideKeysByForegroundKeyAndSignature) {
    const backgroundHexes = segmentBackgroundHexes(segment, paletteTable);
    if (backgroundHexes.length === 0)
        return segment;
    const signature = backgroundSignatureFor(backgroundHexes);
...
function withOverridesAppliedToBlock(rawBlock, paletteTable, overrideKeysByForegroundKeyAndSignature) {
    return withSegmentsTransformed(rawBlock, (segment) => segmentWithOverridesApplied(segment, paletteTable, overrideKeysByForegroundKeyAndSignature));
}
/**
 ...
 * apply.
 */
export function repairSegmentForegrounds(rawBlocks, paletteTable) {
    const normalizedBlocks = rawBlocks.map(withGeneratedBlockForegroundsNormalized);
    const signaturesByForegroundKey = new Map();
    for (const rawBlock of normalizedBlocks) {
        ...
        for (const rawSegment of segments) {
            if (typeof rawSegment === "object" && rawSegment !== null) {
                collectSegmentBackgroundHexes(rawSegment, paletteTable, signaturesByForegroundKey);
            }
        }
    }
 ...
 */
function buildInitLine(shell, ownedConfigPath) {
    if (shell === "pwsh") {
 ...
 */
function findPreexistingInitLineSpan(text, shell, eol) {
    const shellNames = initShellNamesFor(shell);
...
function neutralizedInitLine(line) {
 ...
 */
function withBlockPlacedAt(text, block, eol, initLineSpan) {
 ...
 */
function upsertProfileBlock(text, ownedContent, eol, markerBegin, markerEnd, shell) {
...
function readTextOrEmpty(targetPath) {
 ...
 */
function backupBeforeEdit(targetPath) {
 ...
 */
function buildClinkPromptFilterScript(ownedConfigPath) {
...
function buildProfileOwnedBlock(shell, ownedConfigPath) {
    if (shell === "cmd")
        return buildClinkPromptFilterScript(ownedConfigPath);
 ...
 */
function profileCreationNotice(profilePath, shell) {
 ...
 */
function upsertInitLine(shell, profilePath, ownedConfigPath) {
    if (shell === "cmd" && !detectClink()) {
        throw new Error("Clink is not installed — cmd.exe needs Clink to render through Oh My Posh (https://chrisant996.github.io/clink/); install it and re-run this command");
    }
...
export function defaultOwnedConfigPath() {
 ...
 * whole story.
 */
function noConfigDiscoveredMessage(profilePath, shell) {
 ...
 */
function migrateAwayFromBundledPromptLayout(ownedConfigPath) {
    ...
    try {
        ...
        const originalConfigPath = typeof legacyState === "object" && legacyState !== null ? legacyState["originalConfigPath"] : undefined;
 ...
 */
export function discoverPreOwnedOhMyPoshConfig(profilePath, shell) {
    const discoveredConfigPath = resolveConfigPath(defaultConfigPath(), profilePath, shell);
    if (!discoveredConfigPath || !existsSync(discoveredConfigPath))
        return undefined;
 ...
 */
export function ensureOhMyPoshOwnedConfigSeeded(ownedConfigPath, profilePath, shell) {
    migrateAwayFromBundledPromptLayout(ownedConfigPath);
    if (existsSync(ownedConfigPath))
        return ownedConfigPath;
    const discoveredConfigPath = resolveConfigPath(defaultConfigPath(), profilePath, shell);
 ...
 */
export function reseedOhMyPoshOwnedConfig(sourceConfigPath, ownedConfigPath = defaultOwnedConfigPath()) {
...
function seedStatePathFor(ownedConfigPath) {
...
function writeSeedStateFile(ownedConfigPath, state) {
 ...
 */
function writeOhMyPoshSeedState(ownedConfigPath, seededFromPath) {
 ...
 */
export function readOhMyPoshSeedState(ownedConfigPath = defaultOwnedConfigPath()) {
    const seedStatePath = seedStatePathFor(ownedConfigPath);
    if (!existsSync(seedStatePath))
        return undefined;
    try {
        const parsed = JSON.parse(readFileSync(seedStatePath, "utf8"));
 ...
 */
function recordOriginalPaletteHexes(ownedConfigPath, originalPaletteHexes) {
...
export function ohMyPoshOwnedConfigStatus(ownedConfigPath = defaultOwnedConfigPath()) {
    if (!existsSync(ownedConfigPath))
        return undefined;
...
function unliftableForegroundsNotice(unliftableSegmentTypes) {
    if (unliftableSegmentTypes.length === 0)
        return undefined;
...
function combinedNotice(first, second) {
 ...
 * this ticket.
 */
function originalForeignPalette(currentForeignPalette, knownOriginalHexes) {
 ...
 * exactly that.
 */
function withoutGeneratedOverrideKeys(palette) {
 ...
 */
function recolorConfigInto(configPath, profilePath, shell, scheme) {
    ...
    const paletteTable = recoloredPaletteTable(originalPalette, resolveRoleHexes(scheme), scheme, promptColorsByLiteralForegroundKey(foregroundLift.blocks));
 ...
 */
export function createOhMyPoshAdapter(configPath = defaultOwnedConfigPath(), profilePath = defaultProfilePath(), shell = "pwsh") {
    return {
 ...
 */
export function createDefaultOhMyPoshAdapter() {
...
export function isSegmentType(candidateType) {
...
function roleReferencedBy(segmentPropertyValue) {
 ...
 * rejected" below.
 */
function assertSegmentRolesAreDefined(segment) {
...
function extraBlockProperties(rawBlock) {
 ...
 */
function parseLayoutBlocks(rawBlocks) {
...
function readLayout(configPath) {
 ...
 */
function upsertBlocksArray(configPath, text, blocks) {
    const eol = detectLineEnding(text);
...
function writeLayout(configPath, layout) {
...
export function readOhMyPoshLayout(configPath = defaultOwnedConfigPath()) {
...
export function writeOhMyPoshLayout(layout, configPath = defaultOwnedConfigPath()) {
...
export function buildLayoutSegment(type, foregroundRole, backgroundRole) {
    return {
        type,
...
function blockLabel(alignment, blockIndex) {
...
function assertInsertIndex(atIndex, blockDescription, segmentCount) {
...
function assertExistingIndex(atIndex, blockDescription, segmentCount) {
    if (!Number.isInteger(atIndex) || atIndex < 0 || atIndex >= segmentCount) {
        throw new Error(`index ${atIndex} is out of range for ${blockDescription}, which has ${segmentCount} segment(s)`);
    }
}
/** The blocks sharing `alignment`, in document order — what a `blockIndex` argument counts against. */
function blocksForAlignment(layout, alignment) {
 ...
 */
export function layoutBlocksOnSide(layout, alignment) {
...
function assertBlockIndex(layout, alignment, blockIndex) {
 ...
 */
function withBlockAt(layout, alignment, blockIndex, updatedBlock) {
 ...
 */
function resolveBlockForWrite(layout, alignment, blockIndex) {
    const matchingBlocks = blocksForAlignment(layout, alignment);
    if (blockIndex === matchingBlocks.length) {
        const newBlock = { alignment, segments: [], extra: {} };
 ...
 */
export function addSegment(layout, alignment, blockIndex, segment, atIndex) {
...
export function removeSegment(layout, alignment, blockIndex, atIndex) {
...
export function reorderSegment(layout, alignment, blockIndex, fromIndex, toIndex) {
    const block = assertBlockIndex(layout, alignment, blockIndex);
 ...
 */
export function moveSegmentBetweenBlocks(layout, fromAlignment, fromBlockIndex, fromIndex, toAlignment, toBlockIndex, toIndex) {
 ...
 * business.
 */
export function undoOhMyPosh(configPath = defaultOwnedConfigPath(), profilePath = defaultProfilePath()) {
...
//# sourceMappingURL=oh-my-posh.js.map