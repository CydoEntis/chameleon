...
function writeLockExclusive(lockPath, info) {
    try {
...
function releaseLockIfOwnedByThisProcess(lockPath) {
 ...
 */
export function acquireLock(command, lockPath = defaultLockPath()) {
    ...
    const info = { pid: process.pid, command, acquiredAtMs: Date.now() };
 ...
 */
export function currentLockHolder(lockPath = defaultLockPath()) {
    const holder = readLockInfo(lockPath);
