...
import { claudeCodeSettingsPath, detectShell, herdrConfigPath, ohMyPoshProfilePathFor, stateDir } from "./platform.js";
...
export function defaultOriginalSnapshotPath() {
 ...
 */
function writeSnapshotAtomically(snapshotPath, snapshot) {
 ...
 */
export function readOriginalSnapshot(snapshotPath = defaultOriginalSnapshotPath()) {
...
function captureWindowsTerminalSnapshot(paths) {
 ...
 */
function captureOhMyPoshSnapshot(paths) {
 ...
 */
export function captureOriginalSnapshotIfMissing(snapshotPath = defaultOriginalSnapshotPath(), capturePaths = {}) {
    if (existsSync(snapshotPath))
        return;
    const snapshot = {
...
function writeTextEnsuringDir(targetPath, text) {
...
export function restoreWindowsTerminalFromSnapshot(snapshot) {
    if (!snapshot.windowsTerminal)
        return false;
 ...
 */
export function restoreOhMyPoshFromSnapshot(snapshot) {
    if (!snapshot.ohMyPosh)
        return false;
...
export function restoreHerdrFromSnapshot(snapshot) {
    if (!snapshot.herdr)
        return false;
...
export function restoreClaudeCodeFromSnapshot(snapshot) {
    if (!snapshot.claudeCode)
        return false;
    writeTextEnsuringDir(snapshot.claudeCode.settingsPath, snapshot.claudeCode.settingsText);
    return true;
}
//# sourceMappingURL=original-snapshot.js.map