...
import { ACTIVE_ROW_IDEAL_FRACTION, OVERLAY_0_FRACTION, repairHerdrAccentFamily, repairOverlay0, repairSurface0, resolveActiveRowAndText, resolveHerdrBadgeTokens, resolvePanelBackground, } from "../palette/surfaces.js";
...
const RELOAD_CONFIG_ARGS = ["server", "reload-config"];
/**
 ...
 * either way.
 */
const PACK_SLUG_TO_HERDR_THEME = {
 ...
 */
export const HERDR_BUILTIN_GROUNDS = {
    catppuccin: "#1e1e2e",
 ...
 */
export function nearestHerdrBuiltinThemeNameFor(groundHex) {
    const distances = Object.entries(HERDR_BUILTIN_GROUNDS).map(([themeName, builtinGroundHex]) => ({
        themeName,
 ...
 */
function herdrThemeNameFor(slug, groundHex) {
 ...
 */
export function herdrMatchesScheme(config, scheme) {
...
function assertOnlyAcceptedHerdrTokens(tokenValues) {
 ...
 */
function surfaceScale(groundHex, bodyHex, accentHex, activeRowBackgroundHex, panelBackgroundHex) {
    return {
 ...
 */
function structuralTokenValues(groundHex, activeRowBackgroundHex, bodyHex, accentHex, selectionHex, panelBackgroundHex) {
    return {
...
function detectHerdr(configPath) {
...
function extractStringValue(bodyLines, key) {
...
function extractStringKeyValues(bodyLines) {
    const entries = {};
    for (const line of bodyLines) {
        const trimmedLine = line.trim();
        if (trimmedLine === MARKER_BEGIN || trimmedLine === MARKER_END)
            continue;
        const match = STRING_KEY_VALUE_REGEX.exec(line);
        const key = match?.[1];
        const value = match?.[2];
 ...
 */
function readHerdrConfig(configPath) {
...
function spliceTableBody(lines, table, updatedBodyLines, eol) {
...
function appendTable(text, eol, tableName, bodyLines) {
...
function buildNameLine(themeName) {
 ...
 */
function upsertThemeName(text, eol, themeName) {
    ...
    const updatedBodyLines = existingIndex === -1
...
function buildTokenLines(tokenValues) {
...
function replaceStringValue(line, newValue) {
    return line.replace(STRING_KEY_VALUE_REPLACE_REGEX, (_match, prefix, suffix) => `${prefix}${JSON.stringify(newValue)}${suffix}`);
}
/**
 ...
 */
function takeOverExistingTokenLines(lines, tokenValues) {
...
function tokensNotOwned(tokenValues, ownedTokens) {
 ...
 */
function upsertMarkedTokens(text, eol, tableName, tokenValues) {
    const lines = text.split(eol);
    ...
    if (endIndex === -1) {
        throw new Error(`config.toml has a ch:begin marker in [${tableName}] with no matching ch:end — refusing to guess where Chameleon's block ends`);
    }
...
function resolveHerdrTheme(scheme) {
    const resolvedRoleHexes = resolveRoleHexes(scheme);
    const { selection, body } = resolveSelectionAndBody(scheme.selectionBackground, resolvedRoleHexes.ground, resolvedRoleHexes.body, resolvedRoleHexes.accent, [resolvedRoleHexes.success, resolvedRoleHexes.error]);
    ...
    const rowAndText = resolveActiveRowAndText(resolvedRoleHexes.ground, body.hex, resolvedRoleHexes.muted, [selection.hex, panelBackground.hex], ACTIVE_ROW_IDEAL_FRACTION);
 ...
 */
function customTokenValues(resolvedTheme) {
...
function upsertCustomBlock(text, eol, resolvedTheme) {
 ...
 */
function upsertUiAccent(text, eol, accentHex) {
 ...
 * check of it expects.
 */
function applyHerdrScheme(configPath, scheme, slug) {
 ...
 */
function parseHerdrCliError(stderr) {
...
function describeReloadFailure(result, herdrError) {
    if (result.error) {
 ...
 */
function parseHerdrReloadResult(stdout) {
...
function describeReloadDiagnostics(reloadResult) {
    const { status, diagnostics } = reloadResult.result;
    if (diagnostics.length === 0)
        return `herdr reported status "${status}"`;
...
function reloadHerdr() {
    ...
    if (result.error || result.status !== 0) {
        ...
        if (herdrError?.code === SERVER_NOT_RUNNING_CODE)
            return "Herdr is not running — nothing to reload";
 ...
 */
export function createHerdrAdapter(configPath = defaultConfigPath()) {
    return {
 ...
 */
export function undoHerdr(configPath = defaultConfigPath()) {
    const resolvedConfigPath = requireConfigPath(configPath);
