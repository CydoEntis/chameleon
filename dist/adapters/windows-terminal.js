...
import { buildArrayEntryBlockContent, buildPropertyBlockContent, dedupeConflict, detectLineEnding, findPropertyNode, parseJsonTree, removeNodeFromContainer, requireNode, upsertMarkedBlock, } from "./marked-json-edit.js";
...
function windowsTerminalSchemeName(schemeName) {
 ...
 */
export function selectedFontFace(settings) {
 ...
 * active. See CHM-27.
 */
export function windowsTerminalMatchesScheme(settings, scheme) {
 ...
 */
export function defaultWindowsTerminalSettingsPath() {
    if (!isWindows())
        return undefined;
...
function findSchemeEntryNode(schemesArray, schemeName) {
    return schemesArray.children?.find((entry) => entry.type === "object" && findPropertyNode(entry, "name")?.children?.[1]?.value === schemeName);
}
/**
 ...
 * duplicates it.
 */
function upsertSchemesEntry(settingsPath, text, scheme) {
 ...
 */
function upsertDefaultColorScheme(settingsPath, text, schemeName) {
    const eol = detectLineEnding(text);
    const defaultsNode = requireNode(settingsPath, parseJsonTree(settingsPath, text), ["profiles", "defaults"], "object", 'a "profiles.defaults" object');
    const dedupedText = dedupeConflict(text, defaultsNode, findPropertyNode(defaultsNode, "colorScheme"), "colorScheme");
    const container = requireNode(settingsPath, parseJsonTree(settingsPath, dedupedText), ["profiles", "defaults"], "object", 'a "profiles.defaults" object');
 ...
 */
function upsertTopLevelTheme(settingsPath, text, appearance) {
    const eol = detectLineEnding(text);
 ...
 */
function existingFontShape(defaultsNode) {
    const fontNode = findPropertyNode(defaultsNode, "font");
    if (fontNode?.children?.[1]?.type === "object")
        return "nested";
    if (findPropertyNode(defaultsNode, "fontFace"))
        return "flat";
    return "nested";
}
/**
 ...
 */
function upsertSelectedFont(settingsPath, text, fontFace) {
    const eol = detectLineEnding(text);
    const defaultsNode = requireNode(settingsPath, parseJsonTree(settingsPath, text), ["profiles", "defaults"], "object", 'a "profiles.defaults" object');
...
function detectWindowsTerminal(settingsPath) {
 ...
 */
function readWindowsTerminalSettings(settingsPath) {
 ...
 * untouched.
 */
function applyWindowsTerminalScheme(settingsPath, scheme) {
    ...
    const namedScheme = { ...scheme, name: windowsTerminalSchemeName(scheme.name) };
 ...
 */
function reloadWindowsTerminal() {
 ...
 * settings.json.
 */
export function createWindowsTerminalAdapter(settingsPath = defaultWindowsTerminalSettingsPath()) {
    return {
 ...
 * file's business.
 */
export function selectWindowsTerminalFont(fontFace, settingsPath = defaultWindowsTerminalSettingsPath()) {
 ...
 */
export function undoWindowsTerminal(settingsPath = defaultWindowsTerminalSettingsPath()) {
    const resolvedSettingsPath = requireSettingsPath(settingsPath);
...
function schemeEntryNames(schemesNode) {
...
function removeSchemeEntryByName(settingsPath, text, schemeName) {
    const schemesNode = requireNode(settingsPath, parseJsonTree(settingsPath, text), ["schemes"], "array", 'a "schemes" array');
 ...
 */
export function removeDeadWindowsTerminalSchemeForks(settingsPath = defaultWindowsTerminalSettingsPath()) {
    const resolvedSettingsPath = requireSettingsPath(settingsPath);
    if (!existsSync(resolvedSettingsPath)) {
        throw new Error(`no Windows Terminal settings.json found at ${resolvedSettingsPath}`);
    }
    ...
    if (deadForkNames.length === 0)
        return 0;
    copyFileSync(resolvedSettingsPath, backupPathFor(resolvedSettingsPath));
    const updatedText = deadForkNames.reduce((text, forkName) => removeSchemeEntryByName(resolvedSettingsPath, text, forkName), originalText);
    writeFileAtomically(resolvedSettingsPath, updatedText);
