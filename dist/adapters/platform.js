...
export function isWindows(nodePlatform = process.platform) {
...
export const SHELLS = ["pwsh", "cmd", "bash", "zsh"];
/**
 ...
 */
export function detectShell(nodePlatform = process.platform) {
 ...
 */
export function ohMyPoshProfilePathFor(shell) {
    if (shell === "cmd")
        return clinkScriptPath();
    if (shell === "bash")
...
function powerShellProfilePathFor(documentsDir, edition) {
 ...
 */
export function choosePowerShellEdition(isInstalled, doesProfileExist) {
    if (isInstalled.pwsh && !isInstalled.windowsPowerShell)
        return "pwsh";
    if (isInstalled.windowsPowerShell && !isInstalled.pwsh)
 ...
 */
export function resetPlatformProbeCache() {
    installedByEdition = {};
...
function isPowerShellEditionInstalled(edition) {
 ...
 * binary runs at all.
 */
export function detectPowerShellEdition(documentsDir = windowsDocumentsDir()) {
    const isInstalled = {
...
function expandWindowsEnvironmentReferences(rawPath) {
    return rawPath.replace(/%([^%]+)%/g, (reference, name) => process.env[name] ?? reference);
}
/**
 ...
 */
export function documentsDirFromRegistryQueryOutput(regQueryStdout) {
 ...
 */
export function clinkScriptPath() {
    const localAppData = process.env["LOCALAPPDATA"];
