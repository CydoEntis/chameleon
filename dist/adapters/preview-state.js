...
export function defaultPreviewStatePath() {
 ...
 */
export function readPreviewState(previewStatePath = defaultPreviewStatePath()) {
    if (!existsSync(previewStatePath))
        return undefined;
    try {
        const parsed = JSON.parse(readFileSync(previewStatePath, "utf8"));
...
export function writePreviewState(originalSlug, previewStatePath = defaultPreviewStatePath()) {
    mkdirSync(path.dirname(previewStatePath), { recursive: true });
    const state = originalSlug === undefined ? { updatedAtMs: Date.now() } : { originalSlug, updatedAtMs: Date.now() };
...
export function clearPreviewState(previewStatePath = defaultPreviewStatePath()) {
    if (!existsSync(previewStatePath))
        return;
    rmSync(previewStatePath, { force: true });
}
//# sourceMappingURL=preview-state.js.map