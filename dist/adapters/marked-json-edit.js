...
export function parseJsonTree(sourcePath, text) {
...
export function findPropertyNode(container, key) {
    return container.children?.find((child) => child.type === "property" && child.children?.[0]?.value === key);
}
...
export function requireNode(sourcePath, tree, jsonPath, type, description) {
 ...
 */
export function buildArrayEntryBlockContent(value, eol) {
 ...
 */
export function buildPropertyBlockContent(key, value, eol) {
 ...
 */
function isInsideChild(container, offset) {
 ...
 */
function ownedBlockSpans(text, container) {
    ...
    for (const match of text.slice(containerStart, containerEnd).matchAll(beginPattern)) {
        ...
        if (isInsideChild(container, beginOffset))
            continue;
...
function containerOwnsMarkedBlock(text, container, key) {
 ...
 */
function commaOffsetAfter(text, offset) {
...
function lineEndOffsetInclusive(text, offset) {
 ...
 * owns this" logic.
 */
export function removeNodeFromContainer(text, container, node) {
    ...
    const withoutNode = text.slice(0, lineStartOffset(text, node.offset)) + text.slice(lineEndOffsetInclusive(text, node.offset + node.length));
 ...
 */
export function dedupeConflict(text, container, conflict, key) {
 ...
 */
function withLeadingCommaRestored(text, container, lastSpan) {
    if (!lastSpan)
        return text;
    ...
    if (commaOffsetAfter(text, propertyEndOffset) !== null)
        return text;
 ...
 */
export function upsertMarkedBlock(text, container, ownedContent, eol, key) {
 ...
 */
export function setUnmarkedTopLevelProperty(sourcePath, text, key, value) {
...
//# sourceMappingURL=marked-json-edit.js.map