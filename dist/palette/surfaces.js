 ...
 * this module.
 */
function fractionClearingVisibilityFloor(groundHex, bodyHex, lowFraction, highFraction, minRatio) {
    let low = lowFraction;
 ...
 */
export function resolveActiveRowBackground(groundHex, bodyHex, mutedHex, idealFraction) {
 ...
 * herdr.test.ts).
 */
export function resolvePanelBackground(groundHex, bodyHex) {
 ...
 */
export function repairOverlay0(candidateHex, groundHex, activeRowBackgroundHex, panelBackgroundHex) {
 ...
 */
function fractionReachingLuminance(groundHex, bodyHex, minLuminance) {
    let low = 0;
    let high = 1;
    for (let iteration = 0; iteration < SEARCH_ITERATIONS; iteration += 1) {
        const midFraction = (low + high) / 2;
 ...
 */
export function repairSurface0(candidateHex, groundHex, bodyHex, accentHex) {
    const minChipLuminance = relativeLuminance(accentHex);
    if (relativeLuminance(candidateHex) >= minChipLuminance)
        return keepingTextReadable(candidateHex, groundHex, bodyHex);
    if (relativeLuminance(bodyHex) <= minChipLuminance)
        return keepingTextReadable(bodyHex, groundHex, bodyHex);
...
function keepingTextReadable(chipHex, groundHex, bodyHex) {
    if (contrastRatio(bodyHex, chipHex) >= TEXT_MIN_RATIO)
        return chipHex;
 ...
 */
function lightestFractionClearingTextOnSurface(groundHex, bodyHex) {
    let low = 0;
    let high = 1;
    for (let iteration = 0; iteration < SEARCH_ITERATIONS; iteration += 1) {
        const midFraction = (low + high) / 2;
...
function clearsFloorEverywhere(hex, backgroundHexes, minRatio) {
 ...
 * already was.
 */
function repairMutedForActiveRow(mutedHex, groundHex, bodyHex, activeRowHex) {
 ...
 */
export function resolveActiveRowAndText(groundHex, bodyHex, mutedHex, otherSurfaceHexes, idealFraction) {
...
export function resolveHerdrBadgeTokens(scheme) {
    return {
        blue: scheme.blue,
        teal: scheme.cyan,
        mauve: scheme.purple,
...
export function repairHerdrAccentFamily(roleHexes, badgeTokens, groundHex, panelBackgroundHex) {
...
const WINDOWS_TERMINAL_BACKGROUND_ONLY_FIELDS = new Set(["background", "selectionBackground"]);
/**
 ...
 */
function assertEverySchemeFieldAccountedFor(scheme) {
    const checkedForegroundFields = new Set([...ANSI_SLOT_NAMES, "foreground", "cursorColor"]);
 ...
 * it never repairs.
 */
export function windowsTerminalContrastPairs(scheme) {
    assertEverySchemeFieldAccountedFor(scheme);
    const ansiPairs = ANSI_SLOT_NAMES.map((slotName) => ({
 ...
 */
const HERDR_TOKENS_CARRYING_NO_TEXT = new Set(["surface_dim", "surface0", "surface1", "overlay1"]);
/**
 ...
 */
const HERDR_BACKGROUND_ONLY_TOKENS = new Set(["sidebar_bg", "active_row_bg"]);
...
const HERDR_TEXT_BEARING_SURFACES = ["sidebar_bg", "panel_bg", "active_row_bg"];
...
const HERDR_ACCENT_FOREGROUNDS = ["accent", "green", "red", "yellow", "blue", "teal", "mauve", "peach"];
const HERDR_ACCENT_BACKGROUNDS = ["sidebar_bg", "panel_bg"];
/**
 ...
 */
const HERDR_BADGE_TOKENS = new Set(["yellow", "blue", "teal", "mauve", "peach"]);
/**
 ...
 */
function assertHerdrTokensAccountedFor(tokens) {
    const checkedForegroundTokens = new Set(["text", "subtext0", "overlay0", ...HERDR_ACCENT_FOREGROUNDS, "selection_bg", "panel_bg"]);
    for (const token of Object.keys(tokens)) {
        const isAccountedFor = checkedForegroundTokens.has(token) || HERDR_TOKENS_CARRYING_NO_TEXT.has(token) || HERDR_BACKGROUND_ONLY_TOKENS.has(token);
        if (!isAccountedFor) {
 ...
 */
export function herdrContrastPairs(tokens) {
    assertHerdrTokensAccountedFor(tokens);
    const textBearingPairs = ["text", "subtext0", "overlay0"].flatMap((foregroundToken) => HERDR_TEXT_BEARING_SURFACES.map((backgroundToken) => ({
    ...
    const accentPairs = HERDR_ACCENT_FOREGROUNDS.flatMap((foregroundToken) => HERDR_ACCENT_BACKGROUNDS.map((backgroundToken) => {
        ...
        return {
            ...
            kind: isBadgeToken ? "visibility" : "text",
        };
    }));
    return [
        ...textBearingPairs,
