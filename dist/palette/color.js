...
export function rgbDistance(hexA, hexB) {
    ...
    return Math.sqrt((channelsA.red - channelsB.red) ** 2 +
 ...
 */
export function hueDistanceDegrees(hueA, hueB) {
 ...
 * there.
 */
export function mix(hexA, hexB, fraction) {
    const channelsA = toRgbChannels(hexA);
    const channelsB = toRgbChannels(hexB);
    const mixChannel = (a, b) => clampByte(a + (b - a) * fraction);
...
export function fromHueChromaMatch({ hue, chroma, matchValue }) {
...
export function toHsl(hex) {
    ...
    const hueTurns = max === redFraction
...
function rgbFractionsForHueSextant(huePrime, chroma, secondLargest) {
    if (huePrime < 1)
        return [chroma, secondLargest, 0];
    if (huePrime < 2)
        return [secondLargest, chroma, 0];
    if (huePrime < 3)
        return [0, chroma, secondLargest];
    if (huePrime < 4)
        return [0, secondLargest, chroma];
    if (huePrime < 5)
        return [secondLargest, 0, chroma];
    return [chroma, 0, secondLargest];
}
function toHexPair(fraction, lightnessMatch) {
