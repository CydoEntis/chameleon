import { z } from "zod";
import { isKnownRole, TEXT_MIN_RATIO } from "../constants.js";
...
export function parsePromptLayout(rawJson, fileName) {
...
function roleReferencedBy(fieldValue) {
    if (fieldValue === undefined || !fieldValue.startsWith(PALETTE_REF_PREFIX))
        return undefined;
...
export function findLiteralHexColors(layoutText) {
...
export function findNerdFontGlyphs(layoutText) {
    const matches = layoutText.match(NERD_FONT_GLYPH_PATTERN) ?? [];
    return [...new Set(matches)];
}
function segmentRolePairOf(segment) {
    const unresolvedFieldNames = [];
    if (segment.foreground !== undefined && roleReferencedBy(segment.foreground) === undefined)
        unresolvedFieldNames.push("foreground");
    if (segment.background !== undefined && roleReferencedBy(segment.background) === undefined)
        unresolvedFieldNames.push("background");
    return {
 ...
 */
export function findGroundPairingViolations(layout) {
...
export function lintPromptLayout(layoutText, fileName) {
...
export function assertPromptLayoutIsSafe(layoutText, fileName) {
...
export function countSegments(layout) {
...
function segmentContrastRatio(segment, roleHexes) {
    const { foregroundRole, backgroundRole } = segmentRolePairOf(segment);
    if (foregroundRole === undefined)
        return undefined;
...
export function findContrastFailures(layout, roleHexes, packSlug) {
    return allSegments(layout).flatMap((segment, index) => {
        const ratio = segmentContrastRatio(segment, roleHexes);
        if (ratio === undefined || ratio >= TEXT_MIN_RATIO)
            return [];
...
function resolvedValueFor(value, roleHexes) {
    if (typeof value === "string") {
        const role = roleReferencedBy(value);
        return role !== undefined ? roleHexes[role] : value;
    }
    if (Array.isArray(value))
        return value.map((item) => resolvedValueFor(item, roleHexes));
 ...
 */
export function resolvePromptLayoutRoleReferences(layout, roleHexes) {
    return resolvedValueFor(layout, roleHexes);
}
