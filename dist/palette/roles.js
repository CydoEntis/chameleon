import { GREEN_HUE_MAX_DEGREES, GREEN_HUE_MIN_DEGREES, RED_HUE_MAX_DEGREES, RED_HUE_WRAP_MIN_DEGREES, ROLES, } from "../constants.js";
 ...
 */
export const BASE_COLOR_SLOTS = ["blue", "cyan", "purple", "green", "red", "yellow"];
/**
 ...
 * wrong in that theme.
 */
export function hueCategoryOf(hex) {
...
function toRoleColor(measured, slot, groundHex) {
 ...
 */
function strongestInCategory(measured, groundHex, category) {
    const inCategory = BASE_COLOR_SLOTS.filter((slot) => hueCategoryOf(measured.slots[slot].hex) === category);
 ...
 */
export function assignRolesByContrast(measured) {
    ...
    const assignment = { ground, body, muted, accent, success, error };
    for (const role of ROLES)
        Object.freeze(assignment[role]);
