...
function ratioBetweenLuminances(luminanceA, luminanceB) {
 ...
 */
function luminancesClearingRatio(referenceLuminance, minRatio) {
    ...
    const intervals = [];
    if (darkCeiling >= 0)
        intervals.push({ low: 0, high: Math.min(1, darkCeiling) });
    if (lightFloor <= 1)
        intervals.push({ low: Math.max(0, lightFloor), high: 1 });
    return intervals;
}
/**
 ...
 */
function splitAtPivot(interval, pivot) {
...
function oneSidedPiecesClearingBodyFloor(groundLuminance, bodyLuminance, bodyFloorRatio) {
 ...
 */
function bestGroundRatioAmong(pieces, groundLuminance) {
    return pieces.reduce((best, piece) => Math.max(best, ratioBetweenLuminances(piece.low, groundLuminance), ratioBetweenLuminances(piece.high, groundLuminance)), 1);
}
/**
 ...
 */
function luminanceAtGroundRatio(piece, groundLuminance, targetRatio) {
...
function nearestLuminanceReaching(groundLuminance, bodyLuminance, bodyFloorRatio, targetRatio) {
    const pieces = oneSidedPiecesClearingBodyFloor(groundLuminance, bodyLuminance, bodyFloorRatio);
    const reachingPieces = pieces.filter((piece) => Math.max(ratioBetweenLuminances(piece.low, groundLuminance), ratioBetweenLuminances(piece.high, groundLuminance)) >= targetRatio);
    const candidateLuminances = reachingPieces.map((piece) => luminanceAtGroundRatio(piece, groundLuminance, targetRatio));
    return candidateLuminances.reduce((nearest, candidate) => Math.abs(candidate - groundLuminance) < Math.abs(nearest - groundLuminance) ? candidate : nearest);
}
/**
 ...
 */
function chooseSelectionHue(groundHex, accentHex, otherChromaticHexes) {
    ...
    const fallbackHue = otherChromaticHexes.map((hex) => toHsl(hex).hue).reduce((mostDistant, candidateHue) => hueDistanceDegrees(candidateHue, groundHue) > hueDistanceDegrees(mostDistant, groundHue) ? candidateHue : mostDistant);
 ...
 */
function hueTintedTowardLuminance(hue, chroma, targetLuminance) {
 ...
 */
function isOnTargetSideOfGround(candidateLuminance, targetLuminance, groundLuminance) {
 ...
 * side exists at all.
 */
function maxChromaClearingFloors(hue, targetLuminance, groundHex, bodyHex, bodyFloorRatio, minGroundRatio) {
 ...
 */
function hueTintedAtLuminance(hue, targetLuminance, groundHex, bodyHex, bodyFloorRatio, minGroundRatio) {
...
function bodyAtLuminance(bodyHex, targetLuminance) {
 ...
 */
function widenedBodyLuminance(groundLuminance, bodyLuminance, bodyFloorRatio, targetRatio) {
...
function resolveSelectionAgainstBody(candidateHex, groundHex, bodyHex, accentHex, otherChromaticHexes, bodyFloorRatio, achievableRatio) {
    const groundLuminance = relativeLuminance(groundHex);
 ...
 */
function assertClearsBodyFloor(selectionHex, bodyHex) {
    const ratio = contrastRatio(bodyHex, selectionHex);
    if (ratio < TEXT_MIN_RATIO) {
        throw new Error(`resolveSelectionAndBody produced a selection measuring ${ratio.toFixed(2)} for body-on-selection, below its floor of ${TEXT_MIN_RATIO} — this is a bug in the resolver, not the pack it was given`);
    }
}
/**
 ...
 * on its own.
 */
export function resolveSelectionAndBody(candidateSelectionHex, groundHex, bodyHex, accentHex, otherChromaticHexes) {
    const groundLuminance = relativeLuminance(groundHex);
    const bodyLuminance = relativeLuminance(bodyHex);
    ...
    const nudgedAchievableRatio = bestGroundRatioAmong(oneSidedPiecesClearingBodyFloor(groundLuminance, relativeLuminance(nudgedBodyHex), minAcceptableBodyRatio), groundLuminance);
    const selection = resolveSelectionAgainstBody(candidateSelectionHex, groundHex, nudgedBodyHex, accentHex, otherChromaticHexes, minAcceptableBodyRatio, nudgedAchievableRatio);
