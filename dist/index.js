 ...
 */
import { claudeCodeMatchesAppearance, createClaudeCodeAdapter, describeStatusLine, isClaudeCodeStatusLineEnabled, undoClaudeCode, } from "./adapters/claude-code.js";
import { checkLiveContrastInventory } from "./doctor.js";
...
import { createDefaultOhMyPoshAdapter, createOhMyPoshAdapter, ohMyPoshMatchesScheme, ohMyPoshOwnedConfigStatus, OH_MY_POSH_WINGET_PACKAGE_ID, undoOhMyPosh, } from "./adapters/oh-my-posh.js";
import { captureOriginalSnapshotIfMissing, readOriginalSnapshot, restoreClaudeCodeFromSnapshot, restoreHerdrFromSnapshot, restoreOhMyPoshFromSnapshot, restoreWindowsTerminalFromSnapshot, } from "./adapters/original-snapshot.js";
...
import { createWindowsTerminalAdapter, selectedFontFace, undoWindowsTerminal, windowsTerminalMatchesScheme, WINDOWS_TERMINAL_WINGET_PACKAGE_ID, } from "./adapters/windows-terminal.js";
import { toPalette } from "./palette/palette.js";
import { loadCuratedThemePacks, mergeThemePacksBySlug } from "./palette/theme-pack-library.js";
export const VERSION = "0.0.0";
/** Every target Chameleon can theme. An adapter exists per entry. */
export const TARGETS = ["windows-terminal", "oh-my-posh", "herdr", "claude-code"];
 ...
 * default.
 */
export function loadAllThemePacks(userThemeDir) {
...
function wingetInstallCommand(packageId) {
 ...
 */
function checkTarget(target, isApplicable, isInstalled, wingetPackageId) {
 ...
 */
function currentlySelectedFontFace() {
    try {
 ...
 */
function currentClaudeCodeTheme() {
    try {
 ...
 */
function currentClaudeCodeStatusLine() {
    try {
        const claudeCodeAdapter = createClaudeCodeAdapter();
        if (!claudeCodeAdapter.detect())
            return undefined;
        return {
 ...
 * tests.
 */
export function runDoctorChecks(userThemeDir, statePath, previewStatePath) {
    return {
        targets: [
            checkTarget("windows-terminal", isWindows(), detectSafely(() => createWindowsTerminalAdapter().detect()), WINDOWS_TERMINAL_WINGET_PACKAGE_ID),
...
function adapterForTarget(target) {
    if (target === "windows-terminal")
        return createWindowsTerminalAdapter();
    if (target === "oh-my-posh")
        return createDefaultOhMyPoshAdapter();
 ...
 * never coincide" case.
 */
function combinedDetail(first, second) {
 ...
 */
function applyToTarget(target, scheme, slug) {
    if (target === "oh-my-posh") {
 ...
 * notice (CHM-45).
 */
function runOnInstalledTarget(target, succeededStatus, action) {
...
export function didAnyTargetFail(results) {
...
function findLoadedPack(slug, userThemeDir) {
 ...
 */
export function applyThemePack(slug, userThemeDir, statePath, previewStatePath, originalSnapshotPath) {
 ...
 */
export function previewThemePackToFileTargets(slug, userThemeDir) {
    const loaded = findLoadedPack(slug, userThemeDir);
    const scheme = loaded.pack.payloads["windows-terminal"];
 ...
 */
export function undoAppliedPack(previewStatePath) {
    clearPreviewState(previewStatePath);
 ...
 */
function reloadAfterOriginalRestore(target) {
    if (target === "windows-terminal")
        return createWindowsTerminalAdapter().reload();
    if (target === "oh-my-posh")
        return createDefaultOhMyPoshAdapter().reload();
 ...
 */
function restoreOneTargetFromSnapshot(target, snapshot) {
    if (target === "windows-terminal")
        return restoreWindowsTerminalFromSnapshot(snapshot);
    if (target === "oh-my-posh")
        return restoreOhMyPoshFromSnapshot(snapshot);
    if (target === "claude-code")
        return restoreClaudeCodeFromSnapshot(snapshot);
    return restoreHerdrFromSnapshot(snapshot);
}
function restoreTargetFromSnapshot(target, snapshot) {
    try {
 ...
 * overridden by tests.
 */
export function restoreOriginal(previewStatePath, snapshotPath) {
    clearPreviewState(previewStatePath);
 ...
 * overridden by tests.
 */
export function beginThemePreview(originalSlug, previewStatePath) {
 ...
 */
export function isPreviewInFlight(previewStatePath) {
 ...
 */
export function resyncInterruptedPreview(userThemeDir, statePath, previewStatePath) {
    if (!isPreviewInFlight(previewStatePath))
        return { status: "not-in-flight" };
 ...
 * could fall behind.
 */
function targetMatchesPack(target, payloads, appearance) {
    if (target === "windows-terminal") {
 ...
 * `userThemeDir` is only ever overridden by tests.
 */
export function detectPackDrift(slug, userThemeDir) {
    const loaded = findLoadedPack(slug, userThemeDir);
 ...
 */
function loadedActivePack(userThemeDir, statePath) {
    const state = readActivePackState(statePath);
    if (!state)
        return undefined;
    const { packs } = loadAllThemePacks(userThemeDir);
 ...
 */
export function currentPack(userThemeDir, statePath, previewStatePath) {
 ...
 */
export function activePackRoleHexes(userThemeDir, statePath) {
 ...
 */
export function activePackStatuslineMeterHexes(userThemeDir, statePath) {
 ...
 */
function loadPacksWithActiveIndex(userThemeDir, statePath) {
    const { packs } = loadAllThemePacks(userThemeDir);
    if (packs.length === 0) {
 ...
 */
export function nextPackSlug(userThemeDir, statePath) {
 ...
 * `statePath`, like `userThemeDir`, is only ever overridden by tests.
 */
export function prevPackSlug(userThemeDir, statePath) {
    const { packs, activeIndex } = loadPacksWithActiveIndex(userThemeDir, statePath);
 ...
 * tests.
 */
export function packSlugAtRow(oneBasedRow, userThemeDir) {
    const { packs } = loadAllThemePacks(userThemeDir);
 ...
 * `userThemeDir`, is only ever overridden by tests.
 */
export function findFamilySibling(appearance, userThemeDir, statePath) {
    const state = readActivePackState(statePath);
    if (!state) {
    ...
    const sibling = packs.find((candidate) => candidate.pack.manifest.family === family && candidate.pack.manifest.appearance === appearance);
